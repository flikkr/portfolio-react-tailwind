...
const useMapController = () => {
  ...
  function previous() {
    if (index > 0) {
      ...
      console.log(Story.events[newIdx].coordinates);
      new mapboxgl.Popup()
        .setLngLat(Story.events[newIdx].coordinates)
  ...
  const isLast = () => index === Story.events.length - 1;
  ...
  return [mapContainer, previous, next, centre, isFirst, isLast];
};
